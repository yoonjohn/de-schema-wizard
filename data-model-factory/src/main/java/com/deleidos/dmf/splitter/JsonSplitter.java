...
public class JsonSplitter implements Splitter {
	...
	private String continueLine = "";	// The remaining characters in a line after a json object was finished
	private String type;
	...
	public String parseHeaders() {
		return ""; // Headers don't apply to JSON records
	}
	...
	public String split() {
		...
		// Number of open { which have not yet been closed by a }
		int numOpen = 0;
		...
		while (!done) {
			String temp = "";
			// Reads in a new line if previous line was split completely
			...
			// Continues splitting a previous line if the previous json object was completed mid line
			} else {
				temp = continueLine;
			}
			...
			for (int a = 0; a < temp.length(); a++) {
				...
				if (numOpen == 0) {
					record += temp.substring(0, a + 1);
					continueLine = temp.substring(a + 1);
					return record;
				}
			}
	...
	public JsonSplitter clone() {
		try {
			return (JsonSplitter) super.clone();
		} catch (CloneNotSupportedException e) {
			log.error("Could not clone JsonSplitter", e);
		}
		return null;
	}

	@Override
	@SplitterType(value = "JSON")
